...
export interface SiteRow { id: number; url: string; api_key: string; last_seen: string | null; created_at: string; }
...
sitesRouter.get("/", (async (_req, res) => {
  ...
  if (pool) {
    const [rows] = await pool.query("SELECT id, url, last_seen, created_at FROM wp_manager_sites ORDER BY id DESC");
...
sitesRouter.post("/", (async (req, res) => {
  ...
  try {
    ...
    const resp = await fetch(`${normalized.replace(/\/$/, "")}/wp-json/wpm/v1/status`, {
  ...
  try {
    if (pool) {
      ...
      const [rows] = await pool.query("SELECT id, url, last_seen, created_at FROM wp_manager_sites WHERE url=?", [normalized]);
      return res.status(201).json({ item: Array.isArray(rows) ? rows[0] : rows });
    } else {
      const existing = memory.find((s) => s.url === normalized);
      if (existing) {
        existing.api_key = apiKey; existing.last_seen = lastSeen;
      } else {
        memory.unshift({ id: memory.length + 1, url: normalized, api_key: apiKey, last_seen: lastSeen, created_at: new Date().toISOString() });
      }
